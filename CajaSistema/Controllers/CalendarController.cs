...
namespace CajaSistema.Controllers
{
    public class CalendarController : Controller
    {
        ...
        [HttpPost]
        public JsonResult registrarActividad(string name, DateOnly fecha)
        {
            idUsuarioActivo = HttpContext.Session.GetString("idPersona");

            RegistroActividad _registro = new RegistroActividad();
            _registro.strNombre = name;
            _registro.strFechaRegistro = fecha;
            _registro.strUsuarioRegistro=idUsuarioActivo;
            _registro.strColor = GenerarColorHexAleatorio();

            try
            {
                _appdbContext.registroActividades.Add(_registro);
        ...
        [HttpPost]
        public JsonResult listarActividades(string usuario)
        {
            idUsuarioActivo = HttpContext.Session.GetString("idPersona");
            var listaActividades = _appdbContext.registroActividades.Where(x => x.strUsuarioRegistro == idUsuarioActivo).ToList();
            
            List<AuxActividades> _auxActividades= new List<AuxActividades>();
            foreach (var a in listaActividades)
            {
                AuxActividades auxiliar = new AuxActividades();
                auxiliar.id = a.intIdRegistro;
                auxiliar.name = "Actividad";
                auxiliar.description=a.strNombre;
                auxiliar.date = a.strFechaRegistro;
                auxiliar.type = "event";
                auxiliar.color=a.strColor;

                _auxActividades.Add(auxiliar);
            }
            return Json(_auxActividades);
        }
        ...
        public JsonResult eliminarActividades(int id)
        {
            try
            {
                var actividad = _appdbContext.registroActividades.Where(x => x.intIdRegistro == id).SingleOrDefault();
                _appdbContext.registroActividades.Remove(actividad);
                _appdbContext.SaveChanges();
        ...
        public static string GenerarColorHexAleatorio()
        {
            // Generar un número entre 0x000000 y 0xFFFFFF
            int colorInt = random.Next(0x1000000);
            return $"#{colorInt:X6}";
        }
        ...
        [HttpGet]
        public IActionResult ReporteCalendario(int month, int year)
        {
            idUsuarioActivo = HttpContext.Session.GetString("idPersona");
            ...
            var parameters = new[]
            {
                new SqlParameter("@idPersona",idUsuarioActivo),
